...
# 任意输入主管名字，密码匹配即可
input_user = st.sidebar.text_input("👤 主管姓名", placeholder="请输入您的名字").strip()
correct_password = "20260616"
input_password = st.sidebar.text_input("🔑 登录密码", type="password", placeholder="请输入系统密码")
...
# 登录拦截：未成功登录则不渲染后续页面
if not is_logged_in:
    st.warning("⚠️ 安全提示：请先在左侧边栏输入【主管姓名】和正确密码【20260616】，否则系统处于锁定状态。")
    st.stop()
...
# ==========================================
    ...
    with st.container(border=True):
        ...
        # 补全！！！所有原本包含的完整渠道和清关行名单
        if date_key not in global_db:
            global_db[date_key] = {
                "warehouse": "NJC仓",
                "supervisor": input_user,
                "morning_tasks": [
        ...
        with col_user:
            st.text_input("👤 当班值班主管", value=input_user, disabled=True, key=f"sv_dis_{date_key}")
    ...
    # 检测是否改动过（触发红色未保存警报区）
    is_changed = False
    if (not edited_morning.equals(df_morning) or 
        not edited_customs.equals(df_customs) or 
        not edited_shipping.equals(df_shipping) or 
        not edited_events.equals(df_events)):
        is_changed = True

    if is_changed:
        st.markdown('<div class="status-warning">🚨 警告：检测到当前页表格已被修改！切换日期或关闭网页前，请务必点击下方 [红色保存按钮]，否则新输入的数据将会丢失！</div>', unsafe_allow_html=True)
        st.sidebar.error("🚨 警告：数据未保存！")
    ...
    if all_history:
        for date, content in all_history.items():
            ...
            for row in content.get("customs_data", []):
                try:
                    qty = float(row.get("数量")) if row.get("数量") else 0.0
                except:
                    qty = 0.0
                if row.get("状态") or row.get("数量") or row.get("时间"):
                    customs_rows.append({
                        "日期": date, "仓库": wh, "值班主管": sv,
                        "清关行": row.get("清关行", ""), "状态": row.get("状态", ""),
                        "数量_数值": qty, "提货数量/车数": row.get("数量", ""), "提货到场时间": row.get("时间", "")
                    })
                    
            # 解析发货明细
            for row in content.get("shipping_data", []):
                try:
                    vol = float(row.get("货量")) if row.get("货量") else 0.0
                except:
                    vol = 0.0
                if row.get("货量") or row.get("时间") or row.get("人"):
                    shipping_rows.append({
                        "日期": date, "仓库": wh, "值班主管": sv,
                        "发货渠道": row.get("渠道", ""), "货量_数值": vol, "出库货量(件)": row.get("货量", ""),
    ...
    # ==========================================
    st.markdown("---")
    st.header("📈 NJC 运营仓核心数据可视化看盘")
    
    if customs_rows or shipping_rows:
        col_chart1, col_chart2 = st.columns(2)
        
        with col_chart1:
            st.subheader("📆 最近 7 天吞吐总量趋势走势")
            df_c_temp = pd.DataFrame(customs_rows) if customs_rows else pd.DataFrame(columns=["日期", "数量_数值"])
            df_s_temp = pd.DataFrame(shipping_rows) if shipping_rows else pd.DataFrame(columns=["日期", "货量_数值"])
            
            g_c = df_c_temp.groupby("日期")["数量_数值"].sum().reset_index().rename(columns={"数量_数值": "总提货量"}) if not df_c_temp.empty else pd.DataFrame(columns=["日期", "总提货量"])
            g_s = df_s_temp.groupby("日期")["货量_数值"].sum().reset_index().rename(columns={"货量_数值": "总发货量"}) if not df_s_temp.empty else pd.DataFrame(columns=["日期", "总发货量"])
            
            if not g_c.empty or not g_s.empty:
                df_trend = pd.merge(g_c, g_s, on="日期", how="outer").fillna(0).sort_values("日期").set_index("日期")
                st.line_chart(df_trend, height=260)
            else:
                st.info("数据量不足，无法渲染趋势图")
                
        with col_chart2:
            st.subheader("📦 历史各渠道发货件数大盘对比")
            if shipping_rows:
                df_s_chart = pd.DataFrame(shipping_rows)
                g_s_channel = df_s_chart.groupby("发货渠道")["货量_数值"].sum().reset_index()
                st.bar_chart(g_s_channel.set_index("发货渠道"), height=260, color="#10B981")
            else:
                st.info("暂无发货渠道数据，无法渲染占比图")
    else:
        st.info("💡 提示：目前系统内还没有历史保存记录。请在上方输入数据并点击【💾 保存】后，可视化大盘将会自动生成！")

    # ==========================================
    # 5. 📊 核心数据流水大盘 (严格按照日期倒序排列)
    # ==========================================
    st.markdown("---")
    st.header("📊 NJC 仓运管历史数据总账 (Excel 模式)")
    st.caption("🔥 最新的一天永远会自动置顶在第一行。")
    ...
    if customs_rows or shipping_rows:
        ...
        with panel1:
            ...
            if customs_rows:
                df_c_excel = pd.DataFrame(customs_rows)[["日期", "仓库", "值班主管", "清关行", "状态", "提货数量/车数", "提货到场时间"]]
                # 按日期严格倒序
                df_c_excel = df_c_excel.sort_values(by="日期", ascending=False)
                st.dataframe(df_c_excel, use_container_width=True, hide_index=True)
                
                csv_c = df_c_excel.to_csv(index=False).encode('utf-8-sig')
                st.download_button("📥 导出提货历史 Excel", data=csv_c, file_name=f"NJC提货大盘_倒序_{datetime.date.today()}.csv", mime="text/csv", key="dl_c")
            else:
                st.info("💡 暂无提货流水明细")
        ...
        with panel2:
            ...
            if shipping_rows:
                df_s_excel = pd.DataFrame(shipping_rows)[["日期", "仓库", "值班主管", "发货渠道", "出库货量(件)", "发货交接时间", "现场负责人"]]
                # 按日期严格倒序
                df_s_excel = df_s_excel.sort_values(by="日期", ascending=False)
                st.dataframe(df_s_excel, use_container_width=True, hide_index=True)
                
                csv_s = df_s_excel.to_csv(index=False).encode('utf-8-sig')
                st.download_button("📥 导出发货历史 Excel", data=csv_s, file_name=f"NJC发货大盘_倒序_{datetime.date.today()}.csv", mime="text/csv", key="dl_s")
            else:
                st.info("💡 暂无发货渠道流水明细")
    else:
        st.info("💡 目前数据库暂无历史账目明细。")
